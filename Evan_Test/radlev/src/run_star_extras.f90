...
!            s%use_op_mono_alt_get_kap=.false.
            ...
            !use_op_mono_alt_get_kap = .true.
         else if ((s%star_age>0.1d0) .and. (s%star_age<5d8)) then
                 ...
                 else if ((s%star_age>1d2).and. (s%star_age<1d3)) then
                         s%Pextra_factor=1.5
                 else        
                         s%Pextra_factor = 1.0
                         s%okay_to_reduce_gradT_excess=.false.
                         s%gradT_excess_lambda1=0

                 endif
            s%diffusion_min_Z_for_radaccel=1
            s%diffusion_max_Z_for_radaccel=28
            s%do_element_diffusion=.true.
            s%diffusion_max_T_for_radaccel=5e6
            s%diffusion_screening_for_radaccel = .true.
            s%diffusion_use_full_net=.false.
     !       s%diffusion_num_classes=6
            s%diffusion_v_max=1d2
            s%diffusion_tol_correction_max = 3d-1
            s%diffusion_tol_correction_norm = 3d-3
            s%diffusion_min_X_hard_limit = -5d-3
            !s%diffusion_dt_limit = 1d0 ! no element diffusion if dt < this limit (in seconds)


            s%diffusion_min_dq_at_surface = 1d-18
            ...
            !s%high_logT_op_mono_full_off = 7.0d0
            !s%high_logT_op_mono_full_on = 5.1d0

            s%diffusion_num_classes = 13 ! number of classes of species for diffusion calculations
            s%diffusion_class_representative(1) = 'h1'
            s%diffusion_class_representative(2) = 'he4'
            s%diffusion_class_representative(3) = 'c12'
            s%diffusion_class_representative(4) = 'n14'
            s%diffusion_class_representative(5) = 'o16'
            s%diffusion_class_representative(6) = 'ne20'
            s%diffusion_class_representative(7) = 'mg24'
            s%diffusion_class_representative(8) = 'si28'
            s%diffusion_class_representative(9) = 's32'
            s%diffusion_class_representative(10) = 'ar36'
            s%diffusion_class_representative(11) = 'ca40'
            s%diffusion_class_representative(12) = 'fe52'
            s%diffusion_class_representative(13) = 'fe56'
            !s%energy_eqn_option='dedt'
            s%diffusion_class_A_max(1) = 1
            s%diffusion_class_A_max(2) = 4
            s%diffusion_class_A_max(3) = 12
            s%diffusion_class_A_max(4) = 14
            s%diffusion_class_A_max(5) = 16
            s%diffusion_class_A_max(6) = 20
            s%diffusion_class_A_max(7) = 24
            s%diffusion_class_A_max(8) = 28
            s%diffusion_class_A_max(9) = 32
            s%diffusion_class_A_max(10) = 36
            s%diffusion_class_A_max(11) = 40
            s%diffusion_class_A_max(12) = 52
            s%diffusion_class_A_max(13) = 80
         !else if (s%star_age>1d5) then
            !s%Pextra_factor=1
                if (abs(s%star_mdot)>1d-8) then
                        s%low_logT_op_mono_full_on=5.8d0
                        s%low_logT_op_mono_full_off=5.6d0
                else
                        s%low_logT_op_mono_full_on=4.8d0
                        s%low_logT_op_mono_full_off=4.6d0
                endif
                    
            s%high_logT_op_mono_full_off =6.5d0
            s%high_logT_op_mono_full_on = 6.3d0
            s%op_mono_min_X_to_include = 1d-4 ! skip iso if mass fraction < this
